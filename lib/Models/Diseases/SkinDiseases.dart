...
import 'ResultDisease/Result_Disease.dart';

class SkinDiseases extends StatefulWidget {
  @override
  State<SkinDiseases> createState() => _SkinDiseasesState();
}

class _SkinDiseasesState extends State<SkinDiseases> {
  final picker = ImagePicker();
  File? img;
  var url = "https://skindiseases-production.up.railway.app/predictApi";
  ...
  Future<void> upload() async {
    ...
    if (response.statusCode == 200) {
      ...
      if (resJson != null && resJson['Error'] != null) {
        ...
        Navigator.push(
          context,
          MaterialPageRoute(builder: (context) => ResultDisease(result: "", image: null)),
        );
      } else {
        ...
        if (result.isNotEmpty) {
          ...
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (context) => ResultDisease(result: result, image: img),
            ),
          );
        } else {
          Navigator.push(
